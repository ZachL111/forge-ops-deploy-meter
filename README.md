...
forge-ops-deploy-meter is a JavaScript project for automation. It focuses on this technical goal: Develop a JavaScript command-oriented project for deploy scenarios with append-only fixtures, checkpoint recovery checks, and single-node deterministic mode.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 172, risk penalty 7, latency penalty 3, and weight bonus 6. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
